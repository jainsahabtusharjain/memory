...
def get_vector_store_reference():
    """
    Get the vector store reference string in format: 'provider:collection_name'
    This is used to store in SQLite's vector column as a reference.
    Returns: str like 'qdrant:openmemory' or 'chroma:openmemory'
    """
    # Detect vector store provider and collection name from environment or defaults
    vector_store_provider = None
    collection_name = "openmemory"  # Default collection name
    
    # Check environment variables to determine provider
    if os.environ.get('QDRANT_HOST') and os.environ.get('QDRANT_PORT'):
        vector_store_provider = "qdrant"
        # Collection name can be overridden by env var, but defaults to "openmemory"
        collection_name = os.environ.get('QDRANT_COLLECTION', 'openmemory')
    elif os.environ.get('CHROMA_HOST') and os.environ.get('CHROMA_PORT'):
        vector_store_provider = "chroma"
        collection_name = os.environ.get('CHROMA_COLLECTION', 'openmemory')
    elif os.environ.get('WEAVIATE_CLUSTER_URL') or (os.environ.get('WEAVIATE_HOST') and os.environ.get('WEAVIATE_PORT')):
        vector_store_provider = "weaviate"
        collection_name = os.environ.get('WEAVIATE_COLLECTION', 'openmemory')
    elif os.environ.get('REDIS_URL'):
        vector_store_provider = "redis"
        collection_name = os.environ.get('REDIS_COLLECTION', 'openmemory')
    elif os.environ.get('PG_HOST') and os.environ.get('PG_PORT'):
        vector_store_provider = "pgvector"
        collection_name = os.environ.get('PG_COLLECTION', 'openmemory')
    elif os.environ.get('MILVUS_HOST') and os.environ.get('MILVUS_PORT'):
        vector_store_provider = "milvus"
        collection_name = os.environ.get('MILVUS_COLLECTION', 'openmemory')
    elif os.environ.get('ELASTICSEARCH_HOST') and os.environ.get('ELASTICSEARCH_PORT'):
        vector_store_provider = "elasticsearch"
        collection_name = os.environ.get('ELASTICSEARCH_COLLECTION', 'openmemory')
    else:
        # Default to qdrant if no specific provider detected
        vector_store_provider = "qdrant"
        collection_name = os.environ.get('QDRANT_COLLECTION', 'openmemory')
    
    return f"{vector_store_provider}:{collection_name}"
...
def get_default_memory_config():
    ...
    # Detect graph store based on environment variables
    graph_store_provider = None
    graph_store_config = {}
    
    # Check for Neo4j configuration
    if os.environ.get('NEO4J_URI') or (os.environ.get('NEO4J_HOST') and os.environ.get('NEO4J_PORT')):
        ...
        print(f"Auto-detected graph store: {graph_store_provider} with config: {graph_store_config}")
    elif os.environ.get('MEMGRAPH_URI'):
        graph_store_provider = "memgraph"
        graph_store_config = {
            "url": os.environ.get('MEMGRAPH_URI'),
            "username": os.environ.get('MEMGRAPH_USERNAME', 'memgraph'),
            "password": os.environ.get('MEMGRAPH_PASSWORD', 'mem0graph')
        }
        print(f"Auto-detected graph store: {graph_store_provider} with config: {graph_store_config}")
    
    # Auto-detect LLM and embedder provider based on environment variables
    llm_provider = "openai"
    llm_config = {
        "model": "gpt-4o-mini",
        "temperature": 0.1,
    ...
    print(f"[DEBUG] GOOGLE_API_KEY exists: {bool(google_key)}")
    print(f"[DEBUG] GEMINI_API_KEY exists: {bool(gemini_key)}")
    
    if google_key or gemini_key:
        print(f"[DEBUG] Gemini API key detected, switching to Gemini")
        llm_provider = "gemini"
        llm_config = {
        ...
        print(f"Auto-detected LLM provider: {llm_provider}, Embedder provider: {embedder_provider}")
    ...
    # Update vector store embedding dimensions to match embedder
    if embedder_provider == "gemini":
        # Gemini embedder uses 1024 dimensions
        vector_store_config["embedding_model_dims"] = 1024
        print(f"[DEBUG] Updated vector store embedding_model_dims to 1024 for Gemini")
    elif embedder_provider == "openai":
        # OpenAI embedder uses 1536 dimensions (for text-embedding-3-small)
        if "embedding_model_dims" not in vector_store_config:
            vector_store_config["embedding_model_dims"] = 1536
            print(f"[DEBUG] Set vector store embedding_model_dims to 1536 for OpenAI")
    
    # Update config with modified vector_store_config
    config["vector_store"]["config"] = vector_store_config
    
    # Add graph_store if detected
    if graph_store_provider:
        config["graph_store"] = {
        ...
        try:
            ...
            if db_config:
                ...
                if "mem0" in json_config:
                    ...
                    # Get auto-detected providers before checking database
                    auto_detected_llm = config.get("llm", {}).get("provider")
                    auto_detected_embedder = config.get("embedder", {}).get("provider")
                    db_llm = mem0_config.get("llm", {}).get("provider") if mem0_config.get("llm") else None
                    db_embedder = mem0_config.get("embedder", {}).get("provider") if mem0_config.get("embedder") else None
                    
                    # Update LLM configuration if available
                    if "llm" in mem0_config and mem0_config["llm"] is not None:
                        # If auto-detection found Gemini but DB has OpenAI, prioritize auto-detection
                        # (unless user explicitly set it via UI - we'll assume DB config is user's choice if it exists)
                        if auto_detected_llm == "gemini" and db_llm == "openai":
                            # Check if GOOGLE_API_KEY is currently set (fresh auto-detection)
                            if os.environ.get('GOOGLE_API_KEY') or os.environ.get('GEMINI_API_KEY'):
                                print(f"[INFO] Auto-detected Gemini (env var present), keeping auto-detection instead of DB OpenAI config")
                                # Keep auto-detected config - don't override
                            else:
                                # Env var not set, use DB config
                                config["llm"] = mem0_config["llm"]
                                print(f"[INFO] Using LLM provider from database: {db_llm}")
                        else:
                            # Normal case: use DB config (user's choice or matching auto-detection)
                            config["llm"] = mem0_config["llm"]
                            print(f"[INFO] Using LLM provider from database: {db_llm}")
                    ...
                    if "embedder" in mem0_config and mem0_config["embedder"] is not None:
                        # Same logic for embedder: prioritize auto-detection if env var is present
                        if auto_detected_embedder == "gemini" and db_embedder == "openai":
                            if os.environ.get('GOOGLE_API_KEY') or os.environ.get('GEMINI_API_KEY'):
                                print(f"[INFO] Auto-detected Gemini embedder (env var present), keeping auto-detection instead of DB OpenAI config")
                                # Keep auto-detected config - don't override
                                # Also update vector store dimensions
                                if "vector_store" in config and "config" in config["vector_store"]:
                                    config["vector_store"]["config"]["embedding_model_dims"] = 1024
                            else:
                                config["embedder"] = mem0_config["embedder"]
                                print(f"[INFO] Using embedder provider from database: {db_embedder}")
                        else:
                            config["embedder"] = mem0_config["embedder"]
                            print(f"[INFO] Using embedder provider from database: {db_embedder}")
