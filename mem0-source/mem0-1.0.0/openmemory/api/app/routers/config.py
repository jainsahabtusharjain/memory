...
def get_default_configuration():
    ...
    Auto-detects LLM and embedder provider from environment variables:
    - If GOOGLE_API_KEY or GEMINI_API_KEY is set → uses Gemini
    ...
    # Check for Gemini API key (same logic as memory.py)
...
def get_config_from_db(db: Session, key: str = "main"):
    ...
    config_value = config.value.copy() if config.value else {}
    default_config = get_default_configuration()
    
    # Merge with defaults to ensure all required fields exist
    # Only fill missing fields, don't overwrite existing user choices
    ...
    if "mem0" not in config_value:
        ...
        # Get auto-detected providers from defaults
        auto_detected_llm = default_config["mem0"]["llm"]["provider"]
        auto_detected_embedder = default_config["mem0"]["embedder"]["provider"]
        
        # Ensure LLM config exists with defaults
        # Only fill if completely missing or None, preserve user's choices
        if "llm" not in config_value["mem0"] or config_value["mem0"]["llm"] is None:
            config_value["mem0"]["llm"] = default_config["mem0"]["llm"]
        else:
            db_llm = config_value["mem0"]["llm"].get("provider")
            
            # Apply same logic as get_memory_client(): prioritize auto-detection if env vars exist
            if auto_detected_llm == "gemini" and db_llm == "openai":
                # Check if GOOGLE_API_KEY is currently set (fresh auto-detection)
                if os.environ.get('GOOGLE_API_KEY') or os.environ.get('GEMINI_API_KEY'):
                    # Prioritize auto-detected Gemini over stale DB OpenAI config
                    config_value["mem0"]["llm"] = default_config["mem0"]["llm"]
                else:
                    # Env var not set, use DB config
                    db_llm_config = config_value["mem0"]["llm"].get("config", {})
                    default_llm_config = default_config["mem0"]["llm"].get("config", {})
                    
                    # Ensure required fields exist in LLM config (merge missing fields)
                    for field in ["max_tokens", "temperature", "model"]:
                        if field not in db_llm_config and field in default_llm_config:
                            db_llm_config[field] = default_llm_config[field]
            else:
                ...
                # Ensure required fields exist in LLM config (merge missing fields)
                if "config" not in config_value["mem0"]["llm"]:
                    config_value["mem0"]["llm"]["config"] = {}
            ...
            if config_value["mem0"]["llm"].get("provider") is None:
                config_value["mem0"]["llm"]["provider"] = default_config["mem0"]["llm"]["provider"]
        ...
        if "embedder" not in config_value["mem0"] or config_value["mem0"]["embedder"] is None:
            ...
            db_embedder = config_value["mem0"]["embedder"].get("provider")
