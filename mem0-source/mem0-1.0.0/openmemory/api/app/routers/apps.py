...
@router.get("/")
async def list_apps(
    user_id: Optional[str] = Query(None, description="Optional: User ID to filter apps by owner. If not provided, returns all apps."),
    ...
    # Base query - filter by user owner if user_id provided
    query = db.query(
        App,
    ...
    # Apply user filter only if user_id is provided
    if user_id:
