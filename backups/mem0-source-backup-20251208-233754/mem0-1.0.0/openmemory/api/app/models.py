...
# Conditional import to prevent server crash if categorization unavailable
try:
    from app.utils.categorization import get_categories_for_memory
except Exception as e:
    import logging
    logging.warning(f"Categorization feature not available: {e}. Memory categorization will be disabled.")
    # Fallback function that returns empty list
    def get_categories_for_memory(memory: str) -> list:
        return []
...
def categorize_memory(memory: Memory, db: Session) -> None:
    """Categorize a memory using the configured LLM provider and store the categories in the database."""
    try:
        # Get categories using configured LLM provider (Gemini, OpenAI, Ollama, etc.)
