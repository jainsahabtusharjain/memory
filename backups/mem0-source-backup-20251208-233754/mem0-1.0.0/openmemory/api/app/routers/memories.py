...
async def list_memories(
    ...
    query = query.options(joinedload(Memory.app)).options(joinedload(Memory.categories))
    ...
    # Filter by permissions first (before pagination for accuracy)
    all_memories = query.distinct().all()
    permission_filtered = [
    ...
    total = len(permission_filtered)
    page = params.page
    size = params.size
    start = (page - 1) * size
    end = start + size
    paginated_items = permission_filtered[start:end]
    ...
    transformed_items = [
        MemoryResponse(
            ...
            app_name=memory.app.name if memory.app else "",
            categories=[category.name for category in memory.categories] if memory.categories else [],
    ...
    try:
        ...
        if isinstance(qdrant_response, dict) and 'results' in qdrant_response:
            ...
            for result in qdrant_response['results']:
                if result['event'] == 'ADD':
                    ...
                    if existing_memory:
                        ...
                        # Update vector reference dynamically
                        existing_memory.vector = get_vector_store_reference()
            ...
            if created_memories:
                ...
                # If results is empty, it might be a duplicate or filtered out
                # Log this case and return an informative message
                logging.warning(f"Memory client returned empty results for text: {request.text[:50]}...")
                return {
                    "error": "Memory was not created. It may be a duplicate or filtered out by the memory system.",
                    "details": "Qdrant returned empty results array"
                }
...
class UpdateMemoryRequest(BaseModel):
    memory_content: str
    user_id: str
    memory_id: Optional[str] = None  # Optional - UI sends it but we use URL param
